...
This chart supports using `agent` configuration to configure multiple agents in JCasC (rather than explicitly configuring podTemplates in JCasC).

Below is an example `values.yaml` file which creates multiple agents in JCasC. This example includes an array of agents (`agentList`) and an object hierarchy of agents (`agentHierarchy`). The JCasC in this example treats `agent` as the "base" agent and merges it into all other agents. In lieu of `merge`, all values could be explicitly defined in agents.
